...
class TestTestExecutionTool:
    ...
    async def test_execute_test_without_output_capture(self, execution_tool):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            result = await execution_tool.execute_test(
                test_path, capture_output=False
            )
...
"""
        ...
        with patch.object(execution_tool.output_parser, 'parse_test_output') as mock_parse:
    ...
    def test_build_pytest_command(self, execution_tool):
        ...
        assert cmd == ["python", "-m", "pytest", "-v", "tests/test_example.py::test_func"]
    ...
    async def test_result_serialization(self, execution_tool):
        ...
        assert isinstance(result_json["started_at"], str)  # Datetime serialized to string